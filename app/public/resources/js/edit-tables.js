...
//clear row gaoke added
function ClearRow(row) {
    for (var j = 1; j < row.cells.length; j++) {  
        row.cells[j].innerHTML = '';
        dataItems = row.cells[j].getAttribute("DataItems"); 
        if (!dataItems) {  
            //如果当前单元格没有指定，则查看当前列是否指定  
            dataItems = row.parentNode.rows[0].cells[j].getAttribute("DataItems");  
        }  
        //alert(dataItems);
        if (dataItems) {
            row.cells[j].setAttribute("Value","0"); 
            row.cells[j].innerHTML = '否';
        }
    }
}
