...
/**
 * Database log writer. Writes out messages and stores them in a table.
 *
 */
class Kohana_Log_Writer_Database extends Log_Writer {
	...
	/**
	 * Writes each of the messages into the `lis_log_crash`
	 *
	...
	public function write(array $messages) 
	{
		...
		foreach ($messages as $message) {
			...
			if(isset($message['additional']['exception'])) {
				if($file === NULL) {
					$file = $message['additional']['exception']->getFile();
				}
				if($line === NULL) {
					$line = $message['additional']['exception']->getLine();
				}
			}
			$query
				->param(':ip', $ip)
